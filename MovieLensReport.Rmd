...
The HarvardX Data Science Capstone MovieLens project is a movie recommendation system using machine learning algorithms made famous by the one million dollar Netflix competition launched on october 2006. At the end of the challenge in August 2009, Netflix awarded the Grand Prize to a developer team “BellKor’s Pragmatic Chaos” with a winning solution that increased the accuracy of the company’s recommendation engine by 10%.

Using the 10M version of MovieLens data split into edx training set and 10% validation set, I used some of the machine learning techniques that went into the winning solution for the Netflix competition. The ML model used with the smallest Root Mean Squared Error(RMSE) was a linear regression model using regularized movieId and userId averages at .
...
The conclusion section and the end includes final thoughts on the MovieLens project and 
suggestions to improve the predictive model using more advanced machine learning algorithms and matrix factorization methods. 
...
Since I amd using R version 3.6.0, there were issues reading the MovieLens files directly from GroupLens so a RDS copy of the edx and validation datasets were provided in a HarvardX_Capstone_MovieLens Google Drive. Link is below. https://drive.google.com/drive/folders/1IZcBBX0OmL9wu9AdzMBFUG8GoPbGQ38D 
...
kable(edx_summary, "latex", booktabs = T, caption = "Summary stats for edx dataset")

edx_unique <- data.table(validation %>% summarize(users = n_distinct(userId),
                                                  movies = n_distinct(movieId)))
kable(edx_unique, "latex", booktabs = T, caption = "Edx dataset - unique userIds and movieIds")

```

```{r}
# Data exploration of MovieLens validation data.

val_head <- head(validation)

kable(val_head, "latex", booktabs = T, caption = "Head of validation dataset")

val_summary <- summary(validation)
kable(val_summary, "latex", booktabs = T, caption = "Summary stats for validation dataset")

val_unique <- data.table(validation %>% summarize(users = n_distinct(userId),
                                                  movies = n_distinct(movieId)))
kable(val_unique, "latex", booktabs = T, caption = "Validation dataset - unique userIds and movieIds")
```

## 4. Methods and Analysis:
###   4.1. Data exploration and visualization 

The edx and validation data tables contain a unix timestamp field for when the user rating was
provided in seconds since Jan. 1, 1970 at UTC. In order to use the variable as a factor for visualization and modeling, I needed to transform it into standard datatime. 
...
userId, movieId, date, and the predictive variable ratings.
...
The baseline model RMSE that I will be trying to beat with more advanced linear regression models is generated by using the average rating $(\mu)$ of 3.51245564078807 on the training_set to be predicted into the test_set.

The resulting RMSE for the baseline model is 1.060054 which is a poor model and likely less predictive than random guessing. I will try to improve on this model using dependent variables
movieId, userId, and date in linear regression models along with regularization methods. 
...
The simple linear regression model that I will generate first uses the rating by movieId bias or the *movie effect* $(m_i)$ dependent variable on the training_set to predict the `rating` $(\hat{y}_i)$ for the test_set.

The *movie-specific effect model* is calculated as follows:
$$\hat{y}_i=\mu + b_i+\varepsilon_i$$
where:

##### * μ the average rating for all movies. 
##### * bi effects or bias, movie-specific effect. 
##### * ε (epsilon) is a random error component which measures how far above or below the True
#####   Regression Line the actual observation of y lies. The mean of ε is zero.

The resulting RMSE from this *movie-specific effect model* on the test_set was an improvement
on the baseline model RMSE at 0.9429615. We can likely do better with adding more dependent variables to the model. 
...
rmse_table <- rbind(rmse_table,
                    data_frame(Method = "Predicted movie ratings regression model using movie-specific effect", RMSE = movie_model))
...
The next regression model that I will generate uses the rating by movieId bias or the *movie effect* $(m_i)$ and userId bias or *user effect* $(u_i)$ dependent variables on the training_set to predict the `rating` $(\hat{y}_i)$ for the test_set.

The *movie-user-specific effect model* is calculated as follows:
$$\hat{y}_{i,u}=\mu + b_i + b_u + \varepsilon_{i,u}$$
where:
...
The resulting RMSE from this *movie-user-specific effect model* on the test_set was an improvement on the *movie-specific effect model* RMSE at 0.8646843. Let's see if we can do better with adding the last dependent variable date to the model. 
...
rmse_table <- rbind(rmse_table,
                    data_frame(Method = "Predicted movie ratings regression model using movie-user-specific effect", RMSE = movie_user_model))

rmse_table %>% knitr::kable(caption = "RMSEs")
```
The last linear regression model that I will generate uses the rating by movieId bias or the *movie effect* $(m_i)$, userId bias or *user effect* $(u_i)$, and date bias or *date effect* $(d_i)$ dependent variables on the training_set to predict the `rating` $(\hat{y}_i)$ for the test_set.

The *movie-user-date-specific effect model* is calculated as follows:
$$\hat{y}_{i,u,d} = \mu + b_i + b_u + b_d + \varepsilon_{i,u,d}$$
where:
...
The resulting RMSE from this *movie-user-date-specific effect model* on the test_set was a neligible improvement on the *movie-user-specific effect model* RMSE at 0.8646637. Next step is
...
rmse_table <- rbind(rmse_table,
                    data_frame(Method = "Predicted movie ratings regression model using movie-user-date-specific effect", RMSE = movie_user_date_model))
...
I followed the same approach for the regularization method for recommendation systemsdescribed by professor Rafa Irizarry in the dsbook sourced from the github page https://rafalab.github.io/dsbook/. 

The formula for the regularization method on the *movie-user-specific effect model* is as follows. 

$$\frac{1}{N} \sum_{u,i} \left(y_{u,i} - \mu - b_i - b_u \right)^2 + \lambda \left(\sum_{i} b_i^2 + \sum_{u} b_u^2\right)$$
The resulting RMSE from this *movie-user-specific effect model* with the regularization method
on the test_set to minimize the RMSE using cross validation to pick a $$\lambda$$. In this case,
the $$\lambda$$ that optimized (minimized) the RMSE was 5.
...
RMSE_reg <- sapply(lambda_values, function(l){
  ...
  bu_reg <- training_set %>% 
    left_join(mi_reg, by="movieId") %>%
    group_by(userId) %>%
    summarize(bu_reg = sum(rating - mi_reg - mu_reg)/(n()+l))
  
  
  predicted_ratings_reg <- test_set %>% 
    left_join(mi_reg, by = "movieId") %>%
    left_join(ui_reg, by = "userId") %>%
...
The minimized RMSE is obtained from the *movie-user-specific effect model* using the regularization method with a $\lambda=5$. 
